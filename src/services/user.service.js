 ...
 */
const getPublicInfoUser = (user, getId = false) => (Object.assign{
	email: user.email,
	name: user.name,
	role: user.role,
 ...
 */
const getPublicInfoUsers = async () => (await getUsers()).map(user => getPublicInfoUser(user, true));
...
/**
	...
	*/
const createUser = async (name, email, password, role = 3) => User.create({ name, email, password })
